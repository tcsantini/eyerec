...
void PupilTrackingMethod::detectAndTrack(const Timestamp& ts, const cv::Mat& frame, Pupil& pupil, TrackingParameters params)
{
    ...
    bool fitsMaxSize = params.userMaxPupilDiameterPx <= 0 || pupil.majorAxis() <= params.userMaxPupilDiameterPx;
    bool fitsMinSize = params.userMinPupilDiameterPx <= 0 || pupil.majorAxis() >= params.userMinPupilDiameterPx;
...
Pupil PupilTrackingMethod::detect(const cv::Mat& frame, DetectionParameters params)
{
...
TrackingParameters PupilTrackingMethod::estimateTemporalROI(const Timestamp& ts, const TrackingParameters &oldParams)
{
    ...
    if (!previousPupils.empty()) {
        ...
        const auto remaining = 0.5 * std::max<int>(params.roi.width, params.roi.height) - trackingRectHalfSide;
        ...
        params.roi &= Rect(mostRecent.center - delta, mostRecent.center + delta);
    }
    return params;
}
